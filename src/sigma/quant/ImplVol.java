...
public class ImplVol {
	...
	boolean is_below_horizon(double x){ return fabs(x) < DENORMALISATION_CUTOFF; } // This weeds out denormalised (a.k.a. 'subnormal') numbers.
	...
	double normalised_intrinsic(double x, double q /* q=±1 */){
	   ...
	   const double b_max = Math.exp(0.5*x), one_over_b_max = 1 / b_max;
	...
	double normalised_black_call_using_erfcx(double h, double t) {
	   ...
	   double b = 0.5 * Math.exp(-0.5*(h*h+t*t)) * ( erfcx_cody(-ONE_OVER_SQRT_TWO*(h+t)) - erfcx_cody(-ONE_OVER_SQRT_TWO*(h-t)) );
	...
	double small_t_expansion_of_normalised_black_call(double h, double t){
	   ...
	   // w := t² , h2 := h²
	   double a = 1+h*(0.5*SQRT_TWO_PI)*erfcx_cody(-ONE_OVER_SQRT_TWO*h), w=t*t, h2=h*h;
	   ...
	   double b = ONE_OVER_SQRT_TWO_PI*exp((-0.5*(h*h+t*t)))*expansion;
	   return Math.abs(Math.max(b,0.0));
	}
...
//	              u  = -h/√2  and   v  =  t/√2 .
	...
	double normalised_black_call_with_optimal_use_of_codys_functions(double x, double s){
	   ...
	   if ( q1 < codys_threshold )
	       if ( q2 < codys_threshold )
	           two_b = Math.exp(0.5*x)*erfc_cody(q1) - Math.exp(-0.5*x)*erfc_cody(q2);
	       else
	           two_b = Math.exp(0.5*x)*erfc_cody(q1) - Math.exp(-0.5*(h*h+t*t))*erfcx_cody(q2);
	   else
	       if ( q2 < codys_threshold )
	           two_b =  Math.exp(-0.5*(h*h+t*t))*erfcx_cody(q1) - Math.exp(-0.5*x)*erfc_cody(q2);
	       else
	           two_b =  Math.exp(-0.5*(h*h+t*t)) * ( erfcx_cody(q1) - erfcx_cody(q2) );
	   return fabs(Math.max(0.5*two_b,0.0));
	}
	...
	double f_lower_map(double x, double s){ 
	   ...
	   if (is_below_horizon(s))
	      return 0;
	   double z=SQRT_ONE_OVER_THREE*fabs(x)/s, Phi=norm_cdf(-z);
	   return TWO_PI_OVER_SQRT_TWENTY_SEVEN*fabs(x)*(Phi*Phi*Phi);
	}
	double d_f_lower_map_d_beta(double x,double s){
	   if (is_below_horizon(s))
	      return 1;
	   double z=SQRT_ONE_OVER_THREE*fabs(x)/s, y = z*z, Phi=norm_cdf(-z);
	...
	double inverse_f_lower_map(double x,double f){
	   return is_below_horizon(f) ? 0 : Math.abs(x/(SQRT_THREE*inverse_norm_cdf( Math.pow( f/(TWO_PI_OVER_SQRT_TWENTY_SEVEN*Math.abs(x)) , 1./3.) ))); 
	}
	...
	double implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(double price, double F, double K, double T, double q /* q=±1 */, int N){
	   double intrinsic = fabs(std::max((q<0?K-F:F-K),0.0));
	   if (price<intrinsic)
